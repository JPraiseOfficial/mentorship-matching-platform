...
import { createAvailabilityDto, getResourceByIdParam } from "../dtos/user.dto";
...
export const getAvailability = async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.status(200).json({message: "Successfully fetched availability", availability });
    } catch (error) {
        console.error("Error fetching availability:", error);
        res.status(500).json({ error: "Failed to fetch availability. Please, try again later." });
    }
};
...
export const deleteAvailability = async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.status(500).json({ error: "Failed to delete availability. Please, try again later." });
    }
};
...
export const deleteAllAvailability = async (req: Request, res: Response) => {
    try {
        ...
        res.status(500).json({ error: "Failed to delete all availability. Please, try again later." });
    }
}